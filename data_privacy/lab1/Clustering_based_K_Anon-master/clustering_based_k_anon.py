...
def clustering_knn(data, k=25):
    ...
    while len(data) >= k:
        ...
        # 步骤2: 调用find_best_knn找到k-1个最近邻
        # find_best_knn(index, k, data) 返回 (cluster, record_index)
        cluster, record_index = find_best_knn(index, k, data)
        
        # 步骤3: 从数据集中移除已聚类的记录
        # 使用列表推导式过滤掉record_index中的索引对应的记录
        data = [data[i] for i in range(len(data)) if i not in record_index]
        
        # 步骤4: 将新聚类添加到clusters列表
        clusters.append(cluster)
    
    # ============================ 残差分配：处理剩余记录 ============================
    # 处理剩余不足k条的记录
    while len(data) > 0:
        # 步骤5: 从数据集中取出一条记录（使用pop()）
        record = data.pop()
        
        # 步骤6: 找到最佳聚类（信息损失最小）
        # 使用 find_best_cluster_iloss(record, clusters) 返回聚类索引
...
def clustering_kmember(data, k=25):
    ...
    K-member聚类算法实现：基于信息损失增量的k-匿名聚类
    """
    clusters = []
    ...
    while len(data) >= k:
        # 步骤1: 找到距离当前种子最远的记录索引
        # 使用 find_furthest_record(r_i, data)
        r_pos = find_furthest_record(r_i, data)
        
        # 步骤2: 从数据集中移除并获取该记录作为新种子
        # 使用 data.pop(r_pos) 获取并移除记录
        r_i = data.pop(r_pos)
        ...
        # 步骤4: 补全聚类生长循环
        while len(cluster) < k:
            # 步骤4.1: 找到使信息损失增量最小的记录索引
            # 使用 find_best_record_iloss_increase(cluster, data)
    ...
    while len(data) > 0:
        ...
        # 步骤8: 找到信息损失增量最小的聚类
        # 使用 find_best_cluster_iloss_increase(record, clusters)
